...
const DatosResultados = () => {
  ...
  resultados["resultadosTTM"].forEach((dictionary) => {
    for (const key in dictionary) {
      let Nombre = key,
        ext1 =
          "Valor de TTM: " +
          (dictionary[key][1] === "Nulo" ? "No hay datos" : dictionary[key][1] + " W/m²K");
      if (dictionary[key][0] === "Si cumplen") {
        arrCumplen.push([Nombre, ext1]);
      } else if (dictionary[key][0] != "Todavía no se procesaron datos") {
        arrNoCumplen.push([Nombre, ext1]);
      }
    }
  });
  resultadosEM110["Transmitancia Térmica Maxima"] = {
    cumplen: arrCumplen,
    noCumplen: arrNoCumplen,
  };
  ...
  const ventPuer = [
    ...resultados["resultadosInfiltraciones"][0],
    ...resultados["resultadosInfiltraciones"][1],
  ];
    ...
    for (const key in dictionary) {
      ...
      datosEnv.forEach((cerr) => {
        if (cerr["id"].toString() === key) {
          ...
          if (cerr["tipo"] === "Ventana") {
            ext1 =
              (otros["es_proyectante"] ? "Es proyectante: Si" : "Es proyectante: No") +
              ", " +
              (otros["es_hermetico"] ? "Es hermética: Si" : "Es hermética: No");
          } else {
            ext1 =
              (otros["es_silicona"] ? "Sellado de silicona: Si" : "Sellado de silicona: No") +
              ", " +
              (otros["es_burletes"] ? "Tiene burletes: Si" : "Tiene burletes: No");
          }
        }
      });
      // Colocar los datos donde corresponden
      if ((dictionary[key] === "Si cumple") & (key != "msg")) {
        arrCumplen.push([Nombre, ext1]);
      } else if (key != "msg") {
        arrNoCumplen.push([Nombre, ext1]);
      }
  ...
  resultadosEM110["Infiltraciones no deseada de aire"] = {
    cumplen: arrCumplen,
    noCumplen: arrNoCumplen,
  };
  ...
  resultados["resultadosCondensacion"].forEach((dictionary) => {
    for (const key in dictionary) {
      ...
      // Colocar los datos donde corresponden
      if ((dictionary[key][0] === "Si cumple") & (key != "msg")) {
        arrCumplen.push([Nombre, ext1]);
      } else if (key != "msg") {
        arrNoCumplen.push([Nombre, ext1]);
      }
  ...
  resultadosEM110["Riesgo de condensación superficial"] = {
    cumplen: arrCumplen,
    noCumplen: arrNoCumplen,
  };
  ...
  resultadosEM110["Incidencia solar en los vanos"] = {
    cumplen: arrCumplen,
    noCumplen: arrNoCumplen,
  };
  ...
  contNoCumplen += arrNoCumplen.length;
  if (contNoCumplen > 0) {
    resultadosEM110["Resultado de la evaluación EM.110"] = {
      cumplen: [],
      noCumplen: [["", "No se cumple con lo establecido en la norma."]],
    };
  } else {
    resultadosEM110["Resultado de la evaluación EM.110"] = {
      cumplen: [["", "Se cumple con lo establecido en la norma."]],
      noCumplen: [],
    };
  }
  ...
  const prepareExcelData = () => {
    try {
      ...
      // Encabezados
      excelStructure.resultados.push(["Categoría", "Estado", "Elemento", "Detalle"]);
      ...
      Object.keys(resultadosEM110).forEach((category) => {
        ...
        if (cumplen.length > 0) {
          cumplen.forEach((row) => {
            excelStructure.resultados.push([category, "Cumple", row[0], row[1]]);
          });
        }

        if (noCumplen.length > 0) {
          noCumplen.forEach((row) => {
            excelStructure.resultados.push([category, "No Cumple", row[0], row[1]]);
          });
        }
      });
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      const wsElementsData = [
        ...
        ...excelData.elementos
          .slice(1)
          .map((row) => row.map((cell) => ({ v: cell, s: dataStyle }))),
      ];
      ...
      const wsEmplazamiento = XLSX.utils.aoa_to_sheet(
        wsTermicosData("emplazamiento", "Concepto", "Valor")
      );
  ...
  const resultadosColumns = [
    { title: "Categoría", dataIndex: "0", key: "0" },
    {
      ...
      key: "1",
      render: (text) => (
        <span style={{ color: text === "Cumple" ? "green" : "red", fontWeight: "bold" }}>
          {text}
        </span>
      ),
    },
  ...
  return (
    <Card id="delete-account">
      <div style={{ marginBottom: 16 }}>
        <Button type="primary" onClick={prepareExcelData} style={{ marginRight: 8 }}>
          Preparar Datos
        </Button>
        <Button type="primary" onClick={exportToExcel} disabled={!excelData}>
