...
const ListaCerramientos = ({ elementosEnvol, setElementosEnvol }) => {
  ...
  // Editar elimnar elementos
  const eliminarElemento = (id) => {
    const nuevo = elementosEnvol.filter((item) => item.id !== id);
    setElementosEnvol(nuevo);
  };
  const editarElemento = (id) => {
    setIsModalOpenEdit(true);

    const selItem = elementosEnvol.find((item) => item.id === id);
    setInputEspesor(selItem.espesor);
    setInputConductividad(selItem.resistencia);
    setInputID(id);
  };

  const handleOkEdit = () => {
    // Agregar la lista de capas
    //console.log("Editar", inputID)
    //console.log("Editar", inputEspesor)
    //console.log("Editar", inputConductividad) // Resitencia
    setElementosEnvol((prev) => {
      //console.log("ANTES", prev);
      const updated = prev.map((el) =>
        el.id === inputID
          ? { ...el, espesor: inputEspesor.toString(), resistencia: inputConductividad.toString() }
          : el
      );
  ...
  const [inputEspesor, setInputEspesor] = useState(0);
  const [inputConductividad, setInputConductividad] = useState(0);
  const handleEspesorChange = (event) => {
    setInputEspesor(event.target.value);
  };
  const handleConductividadChange = (event) => {
    setInputConductividad(event.target.value);
  };
    ...
    >
      ...
      <ArgonBox display="flex">
        <ArgonButton
          variant="text"
          color="dark"
          onClick={() => editarElemento(id)}
          sx={{ fontSize: "0.85rem", minWidth: "auto", padding: 0 }}
        >
  ...
  ListaCerramientos.defaultProps = { elementosEnvol: [], setElementosEnvol: null };
  // Typechecking props for the CategoriesList
  ListaCerramientos.propTypes = {
    elementosEnvol: PropTypes.array,
    setElementosEnvol: PropTypes.func,
  };
  return (
    <Box sx={{ pointerEvents: activarCapasR ? "auto" : "none", opacity: activarCapasR ? 1 : 0.5 }}>
      <Grid container spacing={2} sx={{ height: "274vh" }}>
        <Grid
          item
          lg={14}
          sx={{ ml: "auto", flexGrow: 1, height: "23%", maxHeight: "23%", overflowY: "auto" }}
        >
          <Card sx={{ height: "100%" }}>
            {" "}
            {/* Aseguramos que el Card ocupe el 100% de la altura */}
            ...
            <ArgonBox display="flex" justifyContent="center" alignItems="center" pt={2} px={2}>
              <Grid container alignItems="center" justifyContent="center" spacing={3}>
                <Grid item>
                  {" "}
                  <ArgonButton variant="gradient" color="info" onClick={openModal}>
                    {" "}
                    Agregar composición{" "}
                  </ArgonButton>{" "}
                </Grid>
              ...
              <Modal
                ...
                footer={[
                  <ArgonButton key="cancelar" color="dark" variant="gradient" onClick={closeModal}>
                    Cancelar
                  </ArgonButton>,
                  <ArgonButton
                    key="agregar"
              ...
              >
                <Grid container direction="column" spacing={2} alignItems="center">
                  ...
                  <Grid item>
                    <Grid container spacing={2} alignItems="center" justifyContent="center">
                      <Grid item>
                        <Typography variant="h6">Largo (m):</Typography>
                      </Grid>
                      <Grid item>
                        <TextField
                          value={inputAnchura}
                  ...
                  <Grid item>
                    <Grid container spacing={2} alignItems="center" justifyContent="center">
                      <Grid item>
                        <Typography variant="h6">Alto (m):</Typography>
                      </Grid>
                      <Grid item>
                        <TextField
                          value={inputLongitud}
              ...
              {/* MODAL EDITAR VALORES DE LA ENVOLVENTE */}
              <Modal
                title={
                  <Typography variant="h5" align="center" fontWeight="bold">
                    EDITAR VALORES
              ...
              >
                <Grid container direction="column" spacing={2} alignItems="center">
                  ...
                  <Grid item>
                    <Grid container spacing={2} alignItems="center" justifyContent="center">
                      <Grid item>
                        <Typography variant="h6">Espesor (m):</Typography>
                      </Grid>
                      <Grid item>
                        <TextField
                          value={inputEspesor}
                  ...
                  <Grid item>
                    <Grid container spacing={2} alignItems="center" justifyContent="center">
                      <Grid item>
                        <Typography variant="h6">Resistencia (m²K/W):</Typography>
                      </Grid>
                      <Grid item>
                        <TextField
                          value={inputConductividad}
                          onChange={handleConductividadChange}
                          variant="outlined"
                          type="number"
                          size="small"
                          style={{ width: 100, textAlign: "center" }}
