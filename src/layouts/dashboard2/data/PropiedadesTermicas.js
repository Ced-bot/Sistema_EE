...
const PropiedadesTermicas = ({ agregarTransmitancia, limpiarTransmitancias }) => {
  ...
  useEffect(() => {
    if (!loadMats) {
      ...
      try {
        axios
          .get("https://zwgysz1a6h.execute-api.sa-east-1.amazonaws.com/default/LeerMaterialesConst")
          .then((response) => {
  ...
  const handleChangeSelect1 = (value) => {
    ...
    setDataSelect2(
      dataMats
        .filter((item) => item.grupo.S === value)
        .map((item) => ({ id: item.material_id.S, nombre: item.nombre.S }))
    );
  ...
  const handleChangeSelect2 = (value) => {
    ...
    if ("transmitancia" in detalles) {
      ...
      setDataMaterial({
        calor_esp: detalles.calor_especifico.S,
        densidad: detalles.densidad.S,
        conductividad: detalles.conductividad.S,
      });
    }
  };
        ...
        <Box mb={5.5} ml={4}>
          <Grid item>
            <Grid container alignItems="center" justifyContent="center" spacing={1}>
              <Grid item>
                {" "}
                <Typography variant="h6">Calor específico ĉ (J/kgK):</Typography>{" "}
              </Grid>
              <Grid item>
                {" "}
                <TextField
                  value={dataMaterial.calor_esp}
                  variant="outlined"
                  style={{ width: 100 }}
                  inputProps={{ style: { marginLeft: "-13px", textAlign: "center" } }}
                  disabled
                />{" "}
              </Grid>
            </Grid>
        ...
        <Box mb={5.5} ml={4}>
          <Grid item>
            <Grid container alignItems="center" justifyContent="center" spacing={5.6}>
              <Grid item>
                {" "}
                <Typography variant="h6">Densidad ρ (kg/m3):</Typography>{" "}
              </Grid>
              <Grid item>
                {" "}
                <TextField
                  value={dataMaterial.densidad}
                  variant="outlined"
                  style={{ width: 100 }}
                  inputProps={{ style: { marginLeft: "-13px", textAlign: "center" } }}
                  disabled
        ...
        <Box mb={2} ml={4}>
          <Grid item>
            <Grid container alignItems="center" justifyContent="center" spacing={1.1}>
              <Grid item>
                {" "}
                <Typography variant="h6">Conductividad λ (W/mK):</Typography>{" "}
              </Grid>
              <Grid item>
                {" "}
                <TextField
                  value={dataMaterial.conductividad}
                  variant="outlined"
                  style={{ width: 100 }}
                  inputProps={{ style: { marginLeft: "-13px", textAlign: "center" } }}
                  disabled
        ...
        <Box mb={2} ml={4}>
          <Grid item>
            <Grid container alignItems="center" justifyContent="center" spacing={1.1}>
              <Grid item>
                {" "}
                <Typography variant="h6">Transmitancia U (W/m²K):</Typography>{" "}
              </Grid>
              <Grid item>
                {" "}
                <TextField
                  value={dataMaterial.transmitancia}
                  variant="outlined"
                  style={{ width: 100 }}
                  inputProps={{ style: { marginLeft: "-13px", textAlign: "center" } }}
                  disabled
  ...
  const argregarPropiedades = () => {
    ...
    if (!isNaN(trans) && typeof trans === "number" && Number.isFinite(trans)) {
      ...
      agregarTransmitancia({
        name: nombreMat[1],
        transmitancia: trans.toFixed(4),
        resistencia: resist.toFixed(4),
        espesor: espesor,
      });
    } else {
  ...
  return (
    <div>
      <Card>
        <Box
          sx={{ pointerEvents: activarCapasR ? "auto" : "none", opacity: activarCapasR ? 1 : 0.5 }}
        >
          <Grid container spacing={3} alignItems="center">
            <Grid item xs={9} md={2} lg={14} sx={{ ml: "auto" }}></Grid>
            <Grid item xs={9} md={2} lg={16} sx={{ ml: "auto" }}>
              ...
              >
                <Grid item>
                  {" "}
                  <Typography variant="h6">Grupo de materiales: </Typography>{" "}
                </Grid>
              </Grid>
              <Grid
                container
            ...
            <Grid container alignItems="center" justifyContent="center" spacing={3}>
              <Grid item>
                {" "}
                <ArgonButton variant="gradient" color="warning" onClick={handleConfirm}>
                  {" "}
                  Limpiar&nbsp; <CleaningServicesIcon fontSize="large" />
                </ArgonButton>{" "}
              </Grid>
              <Grid item>
                {" "}
                <ArgonButton variant="gradient" color="info" onClick={argregarPropiedades}>
                  {" "}
                  Agregar&nbsp; <ArrowForwardSharpIcon fontSize="large" />
                </ArgonButton>{" "}
              </Grid>
            </Grid>
