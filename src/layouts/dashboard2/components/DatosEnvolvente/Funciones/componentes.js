...
import { resistenciaVertical, resistenciaHorizontal } from './operaciones';
...
export default function RadioGroupGeneral({agregarElemento, nroElementos, elementosRadio, Labels, Labels2, Image1, Image2, Image3 }) {
  ...
  const handleSelectCalTRChange = (selectedOption) => {
    ...
    if (capasElementoR.length > 0){
      ...
      if(selectedOption === "Horizontal"){
        const resistenciHorizontal = resistenciaHorizontal(capasElementoR, areaTotal, valsEditarR.tipo.includes("con el aire"), valsEditarR.familia);
        ...
        const resistenciVertical = resistenciaVertical(capasElementoR, minElementos, valsEditarR.tipo.includes("con el aire"), valsEditarR.familia);
  ...
  RadioGroupGeneral.defaultProps = {nroElementos:0};
  // Typechecking props for the CategoriesList
  RadioGroupGeneral.propTypes = {
    agregarElemento: PropTypes.func, 
    nroElementos: PropTypes.number, 
    elementosRadio: PropTypes.array, 
    Labels: PropTypes.array, 
    Labels2: PropTypes.array, 
    Image1: PropTypes.string, 
    Image2: PropTypes.string, 
    Image3: PropTypes.string };
  
  // eslint-disable-next-line react/prop-types
  const TwoLineLabel = ({ text }) => {
  // eslint-disable-next-line react/prop-types
  ...
  return (
    <span
      style={{
        ...
        minWidth: '150px', // 👈 asegura espacio para los dos renglones
      }}
    >
      <span>{partes.slice(0, mitad).join(' ')}</span>
      <span>{partes.slice(mitad).join(' ')}</span>
    </span>
  );
};
  return (
    <div>
        ...
        <FormLabel id="demo-row-radio-buttons-group-label"></FormLabel>
            <RadioGroup
                ...
                >
                {elementosRadio.map(({ value, label }, index) => (
